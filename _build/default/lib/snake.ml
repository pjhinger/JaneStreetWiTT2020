...
let move_forward head tail direction =
  (*
    ...
    * Remove first tail list item (last tail position).
    * Append current head position to tail list.
    * Return new snake object with new head and new tail list.

    let new_head = Direction.next_position direction head;
    let new_tail = List.tl_exn tail @ [ head ];
    head, tail
  *)
...
let collides_with_self t =
  (*
    * Iterate every element of t.tail.
    * Define function to check if that element equals the snake head position using Position.equal.
...
let move_forward_and_grow ({ direction; extensions_remaining; head; tail } as t) =
  let snake_head_position = Direction.next_position direction head in 
    if extensions_remaining > 0
    then (
      t.extensions_remaining <- extensions_remaining - 1;
      snake_head_position, tail @ [ head ]
    ) else (
      snake_head_position, List.tl_exn tail @ [ head ]
    )
;;
